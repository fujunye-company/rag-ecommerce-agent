...
# ── 4. 场景4: 对比决策 ───────────────────────────────────
bold "=== 场景4: 对比决策 ==="
PRODUCTS_JSON=$(curl -s "$BASE/products?size=2" 2>/dev/null || true)
PID1=$(python3 -c 'import json,sys; d=json.load(sys.stdin); print(d["data"]["items"][0]["product_id"])' <<< "$PRODUCTS_JSON" 2>/dev/null || echo "")
PID2=$(python3 -c 'import json,sys; d=json.load(sys.stdin); print(d["data"]["items"][1]["product_id"])' <<< "$PRODUCTS_JSON" 2>/dev/null || echo "")
RESP=$(curl -s -X POST "$BASE/products/compare" \
    -H "Content-Type: application/json" \
    -d "{\"product_ids\":[\"$PID1\",\"$PID2\"],\"dimensions\":[]}" 2>/dev/null || true)
...
CART_ADD=$(curl -s -X POST "$BASE/cart/items" \
    -H "Content-Type: application/json" \
    -d "{\"session_id\":\"$CART_SESSION\",\"product_id\":\"$TEST_PID\",\"quantity\":1}" 2>/dev/null || true)
check "POST /cart/items 加购成功" echo "$CART_ADD" | grep -q '"quantity"'
...
# 8c. 修改数量
CART_UPDATE=$(curl -s -X PUT "$BASE/cart/items" \
    -H "Content-Type: application/json" \
    -d "{\"session_id\":\"$CART_SESSION\",\"product_id\":\"$TEST_PID\",\"quantity\":2}" 2>/dev/null || true)
check "PUT /cart/items 修改数量" echo "$CART_UPDATE" | grep -q '"updated"'
...
# 商品详情
DETAIL=$(curl -s "$BASE/products/$PID1" 2>/dev/null || true)
check "GET /products/{id} 返回详情" echo "$DETAIL" | grep -q '"title"'
