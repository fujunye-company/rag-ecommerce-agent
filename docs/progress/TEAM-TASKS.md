...
> 生成时间：2026-05-20 | 最后更新：2026-05-21
> 说明：以下任务已打包为独立工作包，队员可按包领取执行，
>   不依赖 Hermes 实时交互，产出可直接集成到项目主干。
>
> **里程碑映射（M1-M10 统一框架 — v3.0 比赛对齐）**：
> | 包 | 映射里程碑 | MVP/全量 | 说明 |
> |:--:|-----------|:---:|------|
> | 包1 数据工程 | M2/M3 | 🏷️ MVP | 种子数据 + Qdrant 入库 |
> | 包2 评测工程 | M5/M8 | 🏷️ MVP 可并行 | 用例构造 MVP 阶段完成；RAGAS 跑分 M8 |
> | 包3 Prompt工程 | M3/M9 | 🏷️ MVP | 基线模板 M3 使用；A/B 优化 M9 |
> | 包4 前端工程 | M4 | 🏷️ MVP | Android UI 对齐设计规范 |
> | 包5 Embedding | M3 | 🏷️ MVP 可选 | 选型对比实验 |
> | 包6 Reranker | M3 | 🏷️ MVP 可选 | 重排序模块，独立接口可并行 |
> | 包7 商品排序 | M3/M4 | 🏷️ MVP 可选 | 多维加权打分，纯业务逻辑 |
> | **包8 购物车** | **M7** | **🏷️ 加分** | **Cart模型+API+Agent+Android组件** |
> | **包9 数据扩充** | **M5** | **🏷️ P0** | **14→50+商品，覆盖6+品类** |
...
```
队员A: 数据工程      → 包1 商品知识库构建 (P0) + 包9 数据扩充 (P0)
队员B: 评测工程      → 包2 评测用例库+自动化管线 (P0)
队员C: Prompt工程    → 包3 Prompt模板精调与验证 (P0)
队员D: 前端工程      → 包4 Android界面对齐UI设计 (P1) + 包8 购物车组件 (P1)
队员A/B可选:        → 包6 Reranker重排序模块 (P1)
队员C/D可选:        → 包7 商品多维排序模块 (P1)
全员可选:           → 包5 Embedding选型验证 (P2)
```
